...
% select LDP function6
LDP = ParameterMethodMLtoTS8("atest_netSeq");
MyLDPfunction       = @ApplyMethodMLtoTS8; % [LDP_v3/ApplyMethodMLtoTS8]
...
for iSeed = 1:nSeed    
    ...
    [REWS,REWS_f,~]                = LDP_v3(time,isValid,beamID,lineOfSightWindSpeed,DT,LDP);

    % Calculate REWS from the lidar data with ML
    REWS_b                = MyLDPfunction(lineOfSightWindSpeed,LDP);
    ...
    plot(time,  REWS);
    ylabel('REWS [m/s]');
    legend('wind field','lidar estimate by LDP_v3 no Ghost')
    subplot(312)
    hold on; grid on; box on
    plot(time,  REWS_WindField_shifted);
    plot(time,  REWS_b);
    ylabel('REWS [m/s]');
    legend('wind field shifted','lidar estimate from S2S model')
    subplot(313)
    hold on; grid on; box on
    plot(time,  Error);
    ylabel('error [m/s]');
